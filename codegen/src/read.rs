...
pub fn impl_read(item: &Item) -> TokenStream {
    match item {
    ...
    let match_children: &Vec<_> = &s
        ...
        .map(|f| {
            ...
            if let Some(ty) = f.ty.is_vec() {
                ...
                quote! { #( #tags )|* => #name.push(#ident::read(r, Some(bs))?), }
            } else if let Some(ty) = f.ty.is_option() {
                let ident = ty.get_ident();
                quote! { #( #tags )|* => #name = Some(#ident::read(r, Some(bs))?), }
            } else {
...
fn set_text(s: &Struct) -> Option<TokenStream> {
    if let Some(field) = &s.text_fld {
        let name = &field.name;
        let tag = bytes_str!(s.tag);

        Some(if field.ty.is_cow_str() {
            quote! { #name = Some(Cow::Owned(r.read_text(#tag, &mut Vec::new())?)); }
        } else {
